...
const AddReference =() =>{
    ...
    const onSubmit = async (ev) =>{
        ...
        if(!titleRef.current || titleRef.current.value===''){
            setTitleError('Titre est obligatiore')
        ...
        try {
            ...
            await axiosClient.post('/references/add',formData,{
                headers :{
        ...
        <div className='w-[100%] h-[100%] flex justify-center items-baseline mt-12'>
            ...
            <form className='flex flex-col justify-between mt-0 border-2 rounded border-amber-800 p-7' onSubmit={onSubmit}>
                <div className='flex flex-col justify-between my-[30px]'>
                    <h2 className='text-[40px] font-bold text-amber-700'>Ajouter une référence</h2>
                </div>
                <div className='flex flex-col justify-between gap-5 my-[20px]'>
                    <label htmlFor='name'>Titre de référence <span className='text-red-600 text-[14px]'>*</span></label>
                    <input id='name' type='text' ref={titleRef} placeholder='Saisir le titre de référence' className='border-2 border-amber-800 rounded p-[12px]'/>
